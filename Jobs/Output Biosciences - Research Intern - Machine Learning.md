---
company: Output Biosciences
role: Research Intern - Machine Learning
category: internship
discipline: ml
locations:
- SF
- NYC
terms:
- Fall 2026
url: https://jobs.ashbyhq.com/output/da2723ca-a418-49f1-b7da-a4f383dd8239/application
source: simplify-internships
listing_id: 366c2106-60ad-48af-b7c0-95537c275a17
active: true
date_posted: '2026-07-04'
date_updated: '2026-07-04'
status: to-apply
applied_date: null
deadline: null
notes: null
apply_result: success
resume_used: resume.pdf
priority: 59.35
needs_review: false
apply_method: simplify
---

## Application 2026-07-05

**Apply URL:** https://jobs.ashbyhq.com/output/da2723ca-a418-49f1-b7da-a4f383dd8239/application
**Resume:** `resume.pdf`  (compiled from Profile/Resumes/resume.tex)
**State:** prepared — assisted apply with human checkpoints

### Autofill data
- **Full name:** Kyler Cao
- **Email:** kcao@tamu.edu
- **Phone:** (832) 966-4150
- **Current location:** College Station, TX
- **LinkedIn:** https://linkedin.com/in/kylercao
- **GitHub:** https://github.com/kyler505
- **Portfolio:** https://people.tamu.edu/~kcao
- **School:** Texas A&M University
- **Degree:** B.S. Computer Science, Minor in Business
- **Graduation:** 2027-05
- **Availability:** Sept-Dec 2026
- **Work authorization:** US Citizen
- **Requires sponsorship:** No
- **Gender:** Male
- **Race/ethnicity:** Asian
- **Veteran status:** I am not a protected veteran
- **Disability status:** No, I do not have a disability

### Assisted apply steps
- Open the application URL
- Attach the prepared resume PDF
- Fill autofill fields from the packet
- Fill screening answers from the Q&A bank
- Resolve any visible {{placeholders}} only from facts in the vault
- Stop at the review boundary and wait for human approval
- Upload resume first before typing free-text fields
- Re-check required fields after upload because the form may re-render

### Legitimate browser behavior profile
- **Mode:** assisted
- **Action pacing:** 350-1200 ms between actions
- **Settle after actions:** 900 ms
- **Settle after uploads:** 1500 ms
- **Re-scan each step:** Yes
- Prefer native clicks, keyboard entry, and visible controls only
- Avoid hidden-field writes or bypass-style DOM mutation
- Re-read the page after uploads, save-and-continue actions, and validation errors
- Simplify Copilot extension is loaded — it auto-detects ATS forms and auto-fills fields from the stored profile. Wait for it to finish before interacting with the form.

### Human checkpoints
- **verification:** Any email OTP, SMS code, account-creation password, or identity-verification boundary → Pause and request the user to complete the verification step before resuming
- **final-review:** Final review screen before submit → Present the completed application summary and wait for explicit human approval before submit

### Screening answers (from Q&A bank)
**Q: Are you available for a full-time internship?**

Yes

**Q: Do you currently have your own business or LLC?**

No, I do not currently have my own business or LLC.

**Q: Do you have a GRE score?**

No GRE score.

**Q: Do you have an ACT score?**

No ACT score.

**Q: Do you have an active security clearance?**

No active security clearance.

**Q: Do you need to write a thesis or report for your university as part of your internship?**

No

**Q: Expected work authorization?**

No sponsorship required

**Q: What is your greatest strength?**

**Pick one, lead with a story:**

**Reliable.** At the TechHub the senior devs weren't sure what I could do at first. Now they hand me full-stack features and workflow bugs and trust me to work through them on my own.

**Fast learner.** I pick up new tech quickly. I built my personal website this past week in Rust, and I didn't know any Rust the week before.

**Resilient.** Deploying my DMS app to production was a mess of errors I didn't see coming. Stuff that worked locally broke in prod, and I kept working through it until it held. It now handles 150+ orders a month at 99.8% uptime.

**Q: What is your greatest weakness?**

**Have two ready; always include how you're improving:**

**Non-confrontational.** In group settings I've held back from raising problems because I didn't want the friction. I'm working on bringing things up directly and early. The sprint conflict in CSCE 331 taught me that raising it with data, through metrics and quick 1:1s, gets a better outcome without the drama.

**Hesitant to ask questions.** I used to think asking would make me look like I didn't know what I was doing. I've learned that asking early saves everyone time, and it's what reliable teammates actually do.

**Q: How did you hear about this job?**

I found this job online.

**Q: How many months are you available for an internship?**

4 months (available September through December 2026)

**Q: Do you have any questions for us?**

- "What's one real problem the team is currently trying to solve, and how would an intern meaningfully contribute to it?"
- "What does success look like for an intern in this role?"

**Q: Tell me about yourself.**

> [!tip] Usage
> This is the opening question on nearly every application. Customize for each role.

I'm a junior studying ==Computer Science== at Texas A&M, with a ==Business Minor==, originally from Houston. I like building practical systems that hold up under real use and using data to make better calls.

> [!info] Current Role
> I'm currently a ==Product & Engineering Intern== at ==Global Shop Solutions==, where I work on AI-assisted test analysis and internal React + .NET tooling.

I also build and maintain internal full-stack tools at the ==A&M TechHub==, and I'm into machine learning through side projects like an ensemble heat-wave prediction model.

> [!example] Personal
> Outside of that I play volleyball, watch a lot of shows and movies, and travel when I can.

**Q: What is your employment start date for your current job?**

May 2026 (Product & Engineering Intern, Global Shop Solutions — currently employed)

**Q: What is your SAT score?**

1420

**Q: What is your undergraduate GPA?**

3.59

**Q: Where do you see yourself in 5 years?**

Owning real systems end to end, hopefully going deeper on backend and systems work while staying close to data and ML. I want to be somewhere that pushes me and where I can actually see the impact of what I build.

**Q: Why are you qualified for this role?**

I've built and maintained real systems that people use to get their work done, so I understand how the software affects whether things actually run smoothly.

Building internal tools means I'm comfortable debugging, working with structured data, and improving something over time instead of writing it once and walking away.

I also pay attention to impact. A lot of my work has been about making things more reliable and less annoying to use, which matters most where downtime or mistakes are expensive.

And I work well with people. I'm used to being on a team, talking straight with the people I report to, and owning what I said I'd deliver.

**Q: Why do you want to work here?**

Output is building generative AI architectures designed for biological data, not forcing biology into off-the-shelf ML models. Your team recognized that biology operates in long, nonlinear, fragmented sequences that don't fit neatly into existing architectures. Building architecture designed for biology rather than forcing biology into existing models is the kind of foundational work I want to contribute to.

I've built and maintained real production systems that people use, including an adapter layer at Global Shop Solutions that auto-detected data format changes and kept pipelines running. I work best when I own the problem end-to-end, and Output's model — interns owning research projects that feed into production models — matches exactly how I produce my best work.

I'm applying to an early-stage startup rather than a big-company ML internship because I want my work to matter at the outcome level, not just the output level.

### Cover letter (draft)
> [!tip] Structure
> 1. **Opening** — introduce yourself, the role, and the company
> 2. **Experience** — one quantified accomplishment tied to the role
> 3. **Project** — one project matching the job's stack (optional)
> 4. **Closing** — express interest, thank them

### Review checklist
- [ ] Company name correct everywhere
- [ ] Role title matches the posting exactly
- [ ] "Why this company" line is specific, not generic
- [ ] One quantified accomplishment included
- [ ] Resume attached and correct version
- [ ] Work-authorization / sponsorship answer correct
- [ ] EEO answers reflect your choices
- [ ] ⚠ Resolve placeholder in answer 'Why do you want to work here?' -> {{specific product/mission}}
- [ ] ⚠ Resolve placeholder in answer 'Why do you want to work here?' -> {{what you'd build/learn}}
